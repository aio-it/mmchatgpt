...
AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    ...
    async def chat_sonnet(self, message: Message):
        await self.helper.debug(f"@sonnet from {message.sender_name}")
    ...
    async def chat_opus(self, message: Message):
        await self.helper.debug(f"@opus from {message.sender_name}")
    ...
    async def chat(self, message: Message, model: str = None):
        ...
        await self.helper.debug(f"chat from {message.sender_name} to {model}")
