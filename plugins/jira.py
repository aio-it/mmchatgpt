...
class Jira(PluginLoader):
    ...
    def get_jira_session(self, uid=None):
        if uid is not None:
            try:
                ...
                self.jira_sessions[uid] = JIRA(server=server, basic_auth=(username, token))
    ...
    async def jira_login_help(self, message: Message):
        ...
        if self.users.is_user(message.sender_name):
            self.driver.reply_to(message, f".jira login <server> <username> <token>")
        ...
        if self.users.is_user(message.sender_name):
            ...
            try:
                if jira.myself() is not None:
                    self.driver.reply_to(message, f"logged in to {server} as {username}")
                else:
    ...
    async def jira_assigned(self, message: Message):
        ...
        if self.users.is_user(message.sender_name):
            ...
            try:
                ...
                return
            try:
                issues = self.jira_get_issues(jira,sort_field="updated",sort_asc=True)
                if len(issues) == 0:
                    self.driver.reply_to(message, f"no assigned issues")
                else:
                    for issue in issues:
                        self.driver.reply_to(message, dedent(f"{issue.key} - {issue.fields.summary} - {issue.fields.status} - {issue.fields.created}/{issue.fields.updated}\n{issue.fields.description}"))
            except Exception as e:
